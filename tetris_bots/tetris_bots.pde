/*
  Tetris Bots
  -----------
  This is was orginially made as a multiplayer game of tetris. I later wrote bots to play the game as a test for heuristic AI.
  Most of the functionality has been kept from the two player tetris game and this code can support human players along side bots.
  
  The framerate is set to 60 by default but can be set much higher. The number of bots can be changed in setup
  and the size of the tetris game can be changed by adjusting the zoom value or the size() of the window.
